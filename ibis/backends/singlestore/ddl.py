...
class CreateFunction(BaseDDL):
    ...
    def _singlestore_signature(self):
...
class CreateUDF(CreateFunction):
    def compile(self):
        ...
        if self.func.language == self.func.LANGUAGE_WASM:
            param_line = f"AS WASM"
        elif self.func.language == self.func.LANGUAGE_PYTHON:
            param_line = f"AS PYTHON"
        else:
            raise ValueError(f"Unsupported function language: {self.func.library}'")

        if self.func.type == self.func.TYPE_FILE:
            param_line += f" INFILE '{self.func.library}'"
        elif self.func.type == self.func.TYPE_MODULE:
            param_line += " '{}'".format(base64.b64encode(self.func.library).decode('utf-8'))
        else:
...
class CreateUDA(CreateFunction):
    def compile(self):
        ...
        singlestore_sig = self._singlestore_signature()
        tokens = [f"AS INFILE '{self.func.lib_path}'"]
...
def _singlestore_input_signature(inputs):
    # TODO: varargs '{}...'.format(val)
    return ', '.join(['{} {} NOT NULL'.format(string.ascii_letters[i], type_to_sql_string(x)) for i, x in enumerate(inputs)])
